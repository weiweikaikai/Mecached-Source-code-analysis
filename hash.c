//Hash表是Memcached里面最重要的结构之一，其采用链接法来处理Hash冲突，
//当Hash表中的项太多时，也就是Hash冲突比较高的时候，Hash表的遍历就脱变成单链表，
//此时为了提供Hash的性能，Hash表需要扩容，
//Memcached的扩容条件是当表中元素个数超过Hash容量的1.5倍时就进行扩容，
//扩容过程由独立的线程来完成，扩容过程中会采用2个Hash表，
//将老表中的数据通过Hash算法映射到新表中，每次移动的桶的数目可以配置，
//默认是每次移动老表中的1个桶。
#include "memcached.h"
