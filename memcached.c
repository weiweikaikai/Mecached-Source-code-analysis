/*************************************************************************
	> File Name: memcached.c
	> Author: wk
	> Mail: 18402927708@163.com
	> Created Time: Fri 08 Jul 2016 03:36:12 PM CST
 ************************************************************************/
...
//全局变量struct stats stats的初始化，该全局变量在memcached启动之后，一直使用  
static void stats_init(void) {
...
conn *conn_from_freelist() {
    ...
    if (freecurr > 0) {//freecurr 为静态全局变量
...
bool conn_add_to_freelist(conn *c) {
    ...
    if (freecurr < freetotal) { //freeconns 还有空间
        freeconns[freecurr++] = c;  //直接添加
        ret = false;
    } else {
        /* try to enlarge free connections array */
		//没有多余空间,进行目前容量的2倍进行扩容
...
/** 创建一个新的conn，设置事件处理函数event_handle() */
conn *conn_new(const int sfd, enum conn_states init_state,
    ...
    if (NULL == c) {//如果没有空闲连接
        if (!(c = (conn *)calloc(1, sizeof(conn)))) {//申请一个网络连接的封装conn
...
static void drive_machine(conn *c) {
    ...
    while (!stop) {
        ...
        case conn_listening:
            addrlen = sizeof(addr);
		//master 线程(main) 进入状态机之后执行accept 操作，这个操作也是非阻塞的。
            if ((sfd = accept(c->sfd, (struct sockaddr *)&addr, &addrlen)) == -1) {
				//非阻塞错误码，这个错误码继续等待
                if (errno == EAGAIN || errno == EWOULDBLOCK) {
                    /* these are transient, so don't log anything */
                    stop = true;
                } else if (errno == EMFILE) { //连接超载就是连接数过多
                    if (settings.verbose > 0)
            ...
            /* 设置套接字非阻塞  //已经accept成功，将accept之后的描述符设置为非阻塞的   */
...
#endif
 //设定socket选项，SO_REUSEADDR表示重用地址信息,必须在bind操作之前设置  
...
static int server_sockets(int port, enum network_transport transport,
                          FILE *portnumber_file) {
    if (settings.inter == NULL) {//settings.inter指定的是要绑定的ip地址信息，如果为空，则表示是绑定本机一个ip 
        return server_socket(settings.inter, port, transport, portnumber_file);  //执行监听和绑定操作  
    } else {//如果服务器有多个ip信息，可以在每个(ip,port)上面绑定一个Memcached实例，
		...
		/*list中ip 地址是 192.163.25.1:80;, 192.168.2.1:80分割的
		char * strtok(char *s, const char *delim);用来将字符串分割成一个个片段。
		参数s 指向欲分割的字符串，参数delim 则为分割字符串，当strtok()在参数s 的字符串中发现到参数delim 的分割字符时则会将该字符改为\0 字符。在第一次调用时，strtok()必需给予参数s 字符串，
        ...
        free(list);//不要忘记strdup函数中为list指针用malloc分配了内存必须用free释放，strdup和strcp不一样
        return ret;
    }
}
...
int main (int argc, char **argv) {
    ...
    /** 4. 把缓冲区与流相联, stderr没有相应缓冲区  */
    setbuf(stderr, NULL);

    /* process arguments */
    /** 5. 命令行参数解析 Memcached启动时，有很多配置参数可以选择，这些配置参数严重影响着Memcached的使用，下面分析下这些参数的意义*/
    while (-1 != (c = getopt(argc, argv,
          ...
          "hi"  /* 帮助信息 */
          "r"   /* mcore文件的大小，如果不指定，按系统的最大值进行   */
          "v"   /* 调试信息 */
          "d"   /* 设置以守护进程方式运行 */
          "l:"  /* 绑定的ip信息，如果服务器有多个ip，可以在多个ip上面启动多个Memcached实例，注意：这个不是可接收的IP地址   */
          "u:"  /* memcached运行的用户，如果以root启动，需要指定用户，否则程序错误，退出*/
          "P:"  /* memcached以daemon方式运行时，保存pid的文件路径信息 后面加上文件路径*/
          "f:"  /* 内存的扩容因子，这个关系到Memcached内部初始化空间时的一个变化  */
...
slab_automove（是否自动移动各个slab，如果该选项打开，会有专门的线程来进行slab的调整*/
     ...
     */
     /*
      * 7. 如果用户指定了多个端口，使用一个工作线程服务每一个UDP端口
      */
    ...
    /** 9. 检查端口及协议启动选项，设定settings.udpport和settings.port */
    ...
    if (maxcore != 0) {
         ...
         */
        if (getrlimit(RLIMIT_CORE, &rlim) == 0)  //获取当前Core文件大小的配置值  
		{
			  //变量初始化为无限制  
         ...
         * created at all.
         */
        //再次确认Core文件允许的大小，如果当前的Core文件的大小为0，
		//则不允许Core文件产生，和maxcore!=0不符，程序结束  
     ...
     */
    /** 
     * 11. 检查并设置RLIMIT_NOFILE 
     * RLIMIT_NOFILE决定了系统能够同时打开的文件(包括网络链接)数量，默认值是1024。
	 显然，对于负载量稍大的服务器而言，这个值会偏低。
     * memcached尝试修改系统的RLIMIT_NOFILE。
    ...
    /** 
     * 12. 权限检查。 
     * memcached不允许用root账户运行，如果是root操作，必须通过-u指定其运行用户 
	 //uid==0表示以root运行程序,如果不是使用root操作就不会执行下面代码  
     */
    ...
    /** 
     * 15. 检查lock_memory标志位，该值由-k设定。 
     * 若开启，则将memcached的内存固定在物理内存中，不能交换到swap中 
    ...
    /** 16. 初始化主线程的libevent实例 */
    main_base = event_init();

    /* initialize other stuff */
    /** 17. 初始化状态信息、Hash表、连接池和slab */
    stats_init();
     ...
     */
     /** 18. 忽略SIGPIPE信号
	 忽略PIPE信号，PIPE信号是当网络连接一端已经断开，这时发送数据，
	 会进行RST的重定向，再次发送数据， 会触发PIPE信号，
	 而PIPE信号的默认动作是退出进程，所以需要忽略该信号。*/
	...
	Memcached采用了典型的Master-Worker的线程模式，
	Master就是由main线程来充当，而Worker线程则是通过Pthread创建的*/
    /** 19. 初始化工作线程。由-t设定，默认为4 */
    ...
    /** 21. 如果指定了slab_reassign，创建slab维护线程   */
    if (settings.slab_reassign &&
    ...
    /** 23. 释放特权后创建 unix 模式套接字 */
    if (settings.socketpath != NULL) {
        errno = 0; //多线程应该将errno在栈中重新初始化
    ...
    /** 24. 创建监听套接字，绑定该套接字，然后进行相关初始化 */
    if (settings.socketpath == NULL) {//如果socketpath为空，则表示使用的TCP/UDP,不是使用unix socket 
        const char *portnumber_filename = getenv("MEMCACHED_PORT_FILENAME");//可以从环境变量读取端口文件所在的文件路径 
        ...
        errno = 0;//settings.port表示Memcached采用的是TCP协议，创建TCP Socket，监听并且绑定  
         ...
         */
        /*
         * 初始化顺序：首先创建监听套接字（低端口的套接字可能需要root权限），
         * 然后释放 root 权限，如果设置以守护进程运行 memcached，则 Daemonise it。
         * 然后初始化 libevent 库。
         */
        ...
        /* 如果设置了UDP端口，创建 UDP 监听套接字，并绑定该套接字 
